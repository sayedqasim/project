...
<?php
    try {
        ...
        $prepareorders=$db->prepare("SELECT * FROM orders WHERE userid=(SELECT userid FROM users WHERE email=?)");
        ...
        foreach ($orderrows as $order) {
            ...
            echo "<h5 style='text-align:center;'> Order Status: ".$order['status'] ."</b></h5>";
            ?>
        ...
        if ($order['type']=="Delivery") {
            echo "<br/><h6 style='text-align:center;'> Delivery Charge is <b>0.7 BD</b></h6>";
            $restauranttotal+=0.7;
        }
